...
def do_dry_run(input_file, subdomain, username, password):
  ...
  with open(input_file, mode='r') as csvfile:
      ...
      for row in csv_reader:
        ...
        req = Request('GET', article_url, headers={'Accept': 'application/json', 'content-type':'application/json'}, auth=(username, password))
        prepped = req.prepare()
        resp = s.send(prepped)
        ...
        if not (new_section in sections_checked):
          ...
          req = Request('GET', section_url, headers={'Accept': 'application/json', 'content-type':'application/json'}, auth=(username, password))
          prepped = req.prepare()
          resp = s.send(prepped)
          if resp.status_code == 200:
...
def do_move(input_file, subdomain, username, password):
  ...
  with open(input_file, mode='r') as csvfile:
      ...
      for row in csv_reader:
        ...
        req = Request('PUT', article_url, data=data, headers={'Accept': '*/*', 'content-type':'application/json'}, auth=(username, password))
        prepped = req.prepare()
        resp = s.send(prepped)
        if resp.status_code != 200:
        ...
        req = Request('GET', f'{label_url}?fields=labels', headers={'Accept': 'application/json', 'content-type':'application/json'}, auth=(username, password))
        prepped = req.prepare()
        resp = s.send(prepped)
        if resp.status_code != 200:
          print(f'Error: getting labels. Status code {resp.status_code} getting {label_url}.')
        ...
        req = Request('PUT', label_url, data=data, headers={'Accept': '*/*', 'content-type':'application/json'}, auth=(username, password))
        prepped = req.prepare()
        resp = s.send(prepped)
        if resp.status_code != 200:
